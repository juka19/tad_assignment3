...
df$party <- ifelse(str_detect(df$Sponsor, "R-[A-Z]{2,}"),"Republican", ifelse(str_detect(df$Sponsor,"D-[A-Z]{2,}"),"Democrat", NA))
```
...
dfm %>% colSums() %>% sort(decreasing = TRUE) %>% head(20)
```

#Wordcloud congress 116

```{r}
dfmat4 <- dfm(corpus_subset(corp, Congress == 116), remove = stopwords("english"), remove_punct = TRUE) %>%
    dfm_trim(min_termfreq = 1)

textplot_wordcloud(dfmat4, max_words = 300)
...
```{r}
dfmat5 <- dfm(corpus_subset(corp, Congress == 117), remove = stopwords("english"), remove_punct = TRUE) %>%
    dfm_trim(min_termfreq = 1)

textplot_wordcloud(dfmat5, max_words = 300)
...
```{r}
dfmat2 <- dfm(corp, remove = stopwords("english"), remove_punct = TRUE, groups = corp$party) %>%
...
```{r}
dfmat3 <- dfm(corp, remove = stopwords("english"), remove_punct = TRUE, groups = corp$Congress) %>%
    dfm_trim(min_termfreq = 3)

textplot_wordcloud(dfmat3, comparison = TRUE, max_words = 300,
                   color = c("blue", "red"))
