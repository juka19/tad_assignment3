...
modelpart15 <- dfm(corp_115, remove = stopwords("english"), remove_punct = TRUE, groups = corp_115$party) %>% dfm_trim(min_termfreq = 3)
...
modelpart16 <- dfm(corp_116, remove = stopwords("english"), remove_punct = TRUE, groups = corp_116$party) %>% dfm_trim(min_termfreq = 3)
...
modelpart17 <- dfm(corp_117, remove = stopwords("english"), remove_punct = TRUE, groups = corp_117$party) %>% dfm_trim(min_termfreq = 3)
...
```{r }
dfmatCon <- dfm(corp, remove = stopwords("english"), remove_punct = TRUE, groups = corp$session) %>%
    dfm_trim(min_termfreq = 3)
