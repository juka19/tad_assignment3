...
```{r}
dfmat116 <- dfm(corpus_subset(corp, Congress == 116), remove = stopwords("english"), remove_punct = TRUE) %>%
...
```{r}
dfmat117 <- dfm(corpus_subset(corp, Congress == 117), remove = stopwords("english"), remove_punct = TRUE) %>%
    dfm_trim(min_termfreq = 1)
